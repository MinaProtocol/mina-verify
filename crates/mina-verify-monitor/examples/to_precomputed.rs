...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: to_precomputed <captured.gossipbin>");
    ...
    let protocol_state =
        serde_json::to_value(&header.protocol_state).expect("serialize protocol_state");
    ...
    println!(
        "{}",
        serde_json::to_string(&json).expect("serialize precomputed json")
    );
}
