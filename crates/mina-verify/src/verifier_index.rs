...
/// proof-systems 0.3.0 uses once_cell::sync::OnceCell for the verifier index's
/// `permutation_vanishing_polynomial_m` / `w` fields.
fn once_lock<T>(value: T) -> once_cell::sync::OnceCell<T> {
...
fn finalize(index: VerifierIndex<Fq>) -> VerifierIndex<Fq> {
    ...
    VerifierIndex::<Fq> {
        srs,
        permutation_vanishing_polynomial_m: once_lock(permutation_vanishing_polynomial_m),
        w: once_lock(w),
        endo,
        linearization,
        powers_of_alpha,
